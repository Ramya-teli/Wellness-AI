...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all custom.sh subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*custom\.sh$/)) {
        return true;
      }

      // Allow all customagent.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*customagent\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@custom.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@custom.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
